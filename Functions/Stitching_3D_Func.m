...
elseif strcmp(paramsFMMetric,'EigenVector')
    ...
    Target_vectors=zeros(3,3,size(Target_seed,1));
    Target_values=zeros(3,3,size(Target_seed,1));
    w=params.FE.windowsize+1;
    temp=zeros(2.*w-1);
    for i = 1 : size(Source_seed,1)
        temp(:)=SourceFeatures(i,:);
        temp=imgaussfilt3(temp,2);
        ...
        [Source_vectors(:,:,i),Source_values(:,:,i)] = eig(H);
    end
    for i = 1 : size(Target_seed,1)
        temp(:)=TargetFeatures(i,:);
        temp=imgaussfilt3(temp,2);
        Hxx=temp(w(1)+1,w(2),w(3))+temp(w(1)-1,w(2),w(3))-2.*temp(w(1),w(2),w(3));
        Hyy=temp(w(1),w(2)+1,w(3))+temp(w(1),w(2)-1,w(3))-2.*temp(w(1),w(2),w(3));
        Hzz=temp(w(1),w(2),w(3)+1)+temp(w(1),w(2),w(3)-1)-2.*temp(w(1),w(2),w(3));
        ...
        Hyz=(temp(w(1),w(2)+1,w(3)+1)+temp(w(1),w(2)-1,w(3)-1)-temp(w(1),w(2)+1,w(3)-1)-temp(w(1),w(2)-1,w(3)+1))./4;
        H=[Hxx,Hxy,Hxz;Hxy,Hyy,Hyz;Hxz,Hyz,Hzz];
        [Target_vectors(:,:,i),Target_values(:,:,i)] = eig(H);
    end
    
    for i = 1 : size(Source_seed,1)
        for j = 1 : size(Target_seed,1)
            %hungInput(i,j)=-sum(abs(sum(Source_vectors(:,:,i).*Target_vectors(:,:,j))));
            hungInput(i,j)=-sum(sum(Source_values(:,:,i).*Target_values(:,:,j)));
        end
    end
    hungInput=hungInput./size(SourceFeatures,2);
