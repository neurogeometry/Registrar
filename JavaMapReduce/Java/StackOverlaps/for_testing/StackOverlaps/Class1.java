...
public class Class1 extends MWComponentInstance<Class1>
{
    ...
    /**
     * Maintains information used in calling the <code>StackOverlaps</code> MATLAB 
     *function.
     */
    private static final MWFunctionSignature sStackOverlapsSignature =
        new MWFunctionSignature(/* max outputs = */ 0,
                                /* has varargout = */ false,
                                /* function name = */ "StackOverlaps",
                                /* max inputs = */ 1,
                                /* has varargin = */ false);
    ...
    {
        this(StackOverlapsMCRFactory.newInstance());
    }
    ...
    {
        this(StackOverlapsMCRFactory.newInstance(getPathToComponentOptions(pathToComponent)));
    }
    ...
    {
        this(StackOverlapsMCRFactory.newInstance(componentOptions));
    }
    ...
    {
        try {
            MWMCR mcr = StackOverlapsMCRFactory.newInstance();
            mcr.runMain( sStackOverlapsSignature, args);
            mcr.dispose();
        } catch (Throwable t) {
            t.printStackTrace();
        }
    ...
    /**
     * Provides the interface for calling the <code>StackOverlaps</code> MATLAB function 
     ...
     * @throws MWException An error has occurred during the function call.
     */
    public void StackOverlaps(List lhs, List rhs) throws MWException
    {
        fMCR.invoke(lhs, rhs, sStackOverlapsSignature);
    }
     ...
     * @throws MWException An error has occurred during the function call.
     */
    public void StackOverlaps(Object[] lhs, Object[] rhs) throws MWException
    {
        fMCR.invoke(Arrays.asList(lhs), Arrays.asList(rhs), sStackOverlapsSignature);
    }

    /**
     * Provides the standard interface for calling the <code>StackOverlaps</code> MATLAB function with 
     * 1 input argument.
     ...
     * @throws MWException An error has occurred during the function call.
     */
    public Object[] StackOverlaps(Object... rhs) throws MWException
    {
        Object[] lhs = new Object[0];
        fMCR.invoke(Arrays.asList(lhs), 
                    MWMCR.getRhsCompat(rhs, sStackOverlapsSignature), 
                    sStackOverlapsSignature);
        return lhs;
    }
}
