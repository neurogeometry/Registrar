...
function [SaveLocation,T]=Global_Linear_Transform(StackPositions,FeaturePositions,Transform_Type,DataFolder)
...
elseif strcmp(Transform_Type,'Rigid')
    ...
    gamma=10; % enforcing unitary transforms
    delta=0.1; % overal regularization of shifts
    lambda=0; % individual regularization of shifts
    learning_rate=0.1;
    Max_Iterations=10000;
...
while outlier
    ...
    if M<Dist_tol  %max(temp)<mean(temp)+3*std(temp)
        outlier=false;
    elseif k(iii,jjj)<=5
