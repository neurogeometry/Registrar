...
FeaturePositions_NR = load('MatchedPoints_Non-Rigid.mat');
...
fname_First = dir(['DL101',T_Names{sourceID},'008\']);
fname_First = fname_First(3:end);
fname_First=cell2mat({fname_First.name}');

fname_Second = dir(['DL101',T_Names{targetID},'008\']);
...
for i=1:size(fname_First,1)
    sourcePath = ['DL101',T_Names{sourceID},'008\',fname_First(i,:)];%'E:\Datasets\TimeLaps\Matches\Traces\DL083B001-A001.swc';
    targetPath = ['DL101',T_Names{targetID},'008\',fname_Second(i,:)];%'E:\Datasets\TimeLaps\Matches\Traces\DL083C001-A001.swc';
    
    load(sourcePath)
    AM_Source_temp = AM.optim;
    r_Source_temp = r.optim;
%     R_Source = ones(size(r_Source,1),1);
    
    
    load(targetPath)
    AM_Target_temp = AM.optim;
    r_Target_temp = r.optim;
%     R_Target = ones(size(r_Target_temp,1),1);
