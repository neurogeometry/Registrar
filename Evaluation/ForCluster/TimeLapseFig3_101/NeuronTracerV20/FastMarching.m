...
function [AMlbl,r]=FastMarching(Orig,SVr,N_steps,Max_Known_Dist,Contrast,R_min,R_max,unisotropy,varargin)
...
%Orig=Orig+rand(sizeOrig).*background; % slow without this step
...
r_ind=StartVoxel;
[R, I] = Find_R_fast(Im,SVr,R_min,max(0.2,(R_max-R_min)/10),R_max);
...
while stop_condition==false   
    ...
    elseif myClass.getFlag==4
        ...
        if ~isempty(SVr_new)
            ...
            [R_new, I_new] = Find_R_fast(Im,SVr_new,R_min,max(0.2,(R_max-R_min)/10),R_max);
            R=[R;R_new];
            I=[I,I_new];
            ...
            RL(end+1:end+length(StartVoxel_new))=R_new;
    ...
    %[T,D,L,AML,KT,KnownPoints,NewKnownPoint,exit_flag]=Fast_Marching_Step(Im,T,D,L,AML,KT,KnownPoints,N6_ind,Max_Known_Dist,RL,inf,h2);
    Max_Known_Time=inf;
    ...
    while stop_cond
        ...
        if ~isempty(NewKnownPoint) && ~isinf(min_T)
            ...
            if D(NewKnownPoint)>=max(3*RL(L(NewKnownPoint)),Max_Known_Dist);
                stop_cond=false;
    ...
    if strcmp(exit_flag,'dist') || strcmp(exit_flag,'collision')    
        ...
        [AMlbl,r_ind,R,I,GDpath,R_GDpath,I_GDpath,Labels]=GradientDescent(Im,T,L,KT,AMlbl,r_ind,R,R_min,R_max,I,NewKnownPoint,exit_flag,N6_ind,h1);
        ...
        if ~isnan(sum(GDpath))
            ...
            elseif strcmp(exit_flag,'collision') && nansum(I_GDpath)~=0 && length(I_GDpath)>1
                for ii=1:length(Labels)
                    endd=find(L(GDpath)==Labels(ii),1,'last');
                    start=max(1,endd-ceil(R(r_ind==GDpath(endd)))+1);
                    I_GDpath(start:endd)=NaN;
                end
                CC=nanmean(I_GDpath)/mean(I);
            end
            if isnan(CC)
                CC=1;
            end
            ...
            %AML1=sparse(diag(ones(1,2*length(AML))));
            KT1=zeros(sizeIm1,'uint8'); KT1(GDind1old)=1; KT1(GDind1)=1;
            
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % fast marching
            %[T1,D1,~,~,~,~,~,~]=Fast_Marching_Step(Im1,T1,D1,L1,AML1,KT1,GDind1,N6_ind1,inf,inf,max(T(KnownPoints)),h2);
            Max_Known_Time1=max(T(KnownPoints));
            NHood1=[GDind1+N6_ind1(1);GDind1+N6_ind1(2);GDind1+N6_ind1(3); ...
                GDind1+N6_ind1(4);GDind1+N6_ind1(5);GDind1+N6_ind1(6)];
            NHood1(KT1(NHood1)==1)=[];
            NHood1(Im1(NHood1)==0)=[]; %!!!!!!!!!!!
            NHood1=unique(NHood1);
            
            TrialPoints1=NHood1(~isinf(T1(NHood1)));
            T_TrialPoints1=T1(TrialPoints1);
            ...
            while stop_cond
                if ~isempty(NHood1)
                    ...
                    keep=NHood1(KT1(NHood1)==0 & ~isinf(T1(NHood1)));
                    KT1(keep)=2;
                    TrialPoints1=[TrialPoints1;keep];
                    T_TrialPoints1=[T_TrialPoints1;T1(keep)];
                end
                
                [min_T,min_ind]=min(T_TrialPoints1);
                NewKnownPoint1=TrialPoints1(min_ind);
                
                if ~isempty(NewKnownPoint1) && ~isinf(min_T)
                    KT1(NewKnownPoint1)=1;
                    T_TrialPoints1(min_ind)=inf;
                    NHood1=NewKnownPoint1+N6_ind1;
                    %GDind1=[GDind1;NewKnownPoint1];
                    
                    %tempL=L1(NHood1(KT1(NHood1)==1));
                    %collision=(sum(AML1(L1(NewKnownPoint1),tempL))~=length(tempL));
                    
                    NHood1(KT1(NHood1)==1)=[];
                    NHood1(Im1(NHood1)==0)=[]; %!!!!!!!!!!!
                    
                    if T1(NewKnownPoint1)>=Max_Known_Time1
                        stop_cond=false;
                        %exit_flag='time'; % maximum time reached
                    end
%                     if D1(NewKnownPoint1)>=max(3*RL(L1(NewKnownPoint1)),Max_Known_Dist);
%                         stop_cond=false;
%                         exit_flag='dist'; % maximum distance reached
%                     end
%                     if collision
%                         stop_cond=false;
%                         exit_flag='collision';
%                         AML1(L1(NewKnownPoint1),tempL)=1;
%                         AML1(tempL,L1(NewKnownPoint1))=1;
%                         %AML1(tempL,tempL)=1;
%                     end
                else
                    stop_cond=false;
            ...
            T1(~temp_ind & KT(Xmin:Xmax,Ymin:Ymax,Zmin:Zmax)>0)=inf;
            D1(~temp_ind & KT(Xmin:Xmax,Ymin:Ymax,Zmin:Zmax)>0)=inf;
...
% function [T,D,L,AML,KT,KnownPoints,exit_ind,exit_flag]=Fast_Marching_Step(Im,T,D,L,AML,KT,KnownPoints,N6_ind,Max_Known_Dist,RL,Max_Known_Time,h2)
% 
% NHood=[KnownPoints+N6_ind(1);KnownPoints+N6_ind(2);KnownPoints+N6_ind(3); ...
%     KnownPoints+N6_ind(4);KnownPoints+N6_ind(5);KnownPoints+N6_ind(6)];
% NHood(KT(NHood)==1)=[];
...
% relabeled.
function [AMlbl,r_ind,R,I,GDpath,R_GDpath,I_GDpath,Labels]=GradientDescent(Im,T,L,KT,AMlbl,r_ind,R,R_min,R_max,I,ind,exit_flag,N6_ind,h1)
...
for i=1:length(Labels)
    ...
    if Labels(i)==L(current_ind)
        [~,temp_ind]=nanmax((T(current_ind)-T_pot_ind).*h1);
        temp_v=T_pot_ind(temp_ind);
    else
        [~,temp_ind]=nanmin(T_pot_ind);
        temp_v=0;
    end
    
    while ~isempty(current_ind) && temp_v<T(current_ind)
...
[R_GDpath, I_GDpath] = Find_R_fast(Im_GDpath,r_GDpath,R_min,max(0.2,(R_max-R_min)/10),R_max);
