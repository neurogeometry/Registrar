...
function [Im,AMlbl,r]=FastMarching5(Orig)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% writerObj = VideoWriter('C:\Users\Armen Stepanyants\Desktop\Fast_Marching.avi');
...
N_steps=150; % number of re-initialization steps
...
%SVx=SVx([1,22]); SVy=SVy([1,22]); SVz=SVz([1,22]);
...
drawnow
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% frame = getframe(gca,[1,1,sizeIm(1),sizeIm(2)]);
...
while stop_condition==false && count<=N_steps
    if create_loops
        display([count,2*length(StartVoxel)-(sum(AML(:))-length(AML))/2]);
    elseif ~create_loops
        display([count]);
    end
...
%     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if count>=2
        ...
        if 0 %count>av_window && Q(count)>Qmean+5*Qstd %|| Q1(count)>Q1mean+5*Q1std)
...
hold on
PlotAM(AMlbl, r)
plot3(SVy-pad,SVx-pad,SVz-pad,'r.')
...
while stop_cond
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     ccc=ccc+1;
%     if ~isinf(Max_dist) && mod(ccc,mmm)==0
%         if ccc==360
%             mmm=100;
%         end
%         figure(10)
%         T_RGB=min(T,[],3); T_RGB=(1./(log(T_RGB+1)+1)).^1; 
%         Im_RGB=max(Im,[],3); Im_RGB(T_RGB~=0)=0;
%         T_0=zeros(size(T_RGB)); T_0(T_RGB==1)=1; T_RGB(T_RGB==1)=0;
%         T_0=cat(3,1.*T_0,0.*T_0,0.*T_0);
%         T_RGB=cat(3,0.*T_RGB,1.*T_RGB,0.*T_RGB);
%         Im_RGB=cat(3,Im_RGB,Im_RGB,Im_RGB);
%         imshow(Im_RGB+T_RGB+T_0)
%         axis tight
%         sc=4;
%         %w=get(gcf,'Position')
%         set(gcf,'Position', [1 1 233*sc 155*sc]);
%         drawnow, hold on
%         
%         frame = getframe(gca,[1,1,size(T,1)*sc,size(T,2)*sc]);
%         writeVideo(writerObj,frame);
%     end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
