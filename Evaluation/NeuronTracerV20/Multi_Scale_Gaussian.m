% This is a fast multi-scale Gaussian filter. R is the scale,
% defined as sqrt(2) times the SD of the best Gaussian filter

function Im_max = Multi_Scale_Gaussian(Im,R_min,R_step,R_max,varargin)
...
disp('Multi-scale Gaussian filtering started.')
cl=class(Im);
...
if isempty(s)
    ...
    if(Byte_needed > mem_left)
        ...
        for k=1:length(s)
            ...
            fGx_pad=exp(-Kxp.^2.*s(k)^2./4).*exp(1i.*Kx.*(center-1)); % conjugated fCSF
            fGy_pad=exp(-Kyp.^2.*s(k)^2./4).*exp(1i.*Ky.*(center-1));
            fGz_pad=exp(-Kzp.^2.*s(k)^2./4).*exp(1i.*Kz.*(center-1));
            
            Im_temp=real(ifftn(fIm_pad.*...
                repmat(transpose(fGx_pad),[1,sizeIm_pad(2),sizeIm_pad(3)]).*...
                repmat(fGy_pad,[sizeIm_pad(1),1,sizeIm_pad(3)]).*...
                repmat(reshape(fGz_pad,[1,1,sizeIm_pad(3)]),[sizeIm_pad(1),sizeIm_pad(2),1]))); % correct up to a numerical factor
...
disp('Multi-scale Gaussian filtering is complete.')
