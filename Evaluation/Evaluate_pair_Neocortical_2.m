...
load('../data/StackData_DIADEM2.mat');
        ...
        sourcePath = ['../data/evaluation/DIADEM2/4-3/',temp,'_opt.swc'];%00760_Opt.swc';%
        temp=StackList{targetID,1};
        targetPath = ['../data/evaluation/DIADEM2/4-3/',temp,'_opt.swc'];%00759_Opt.swc';%
...
%     correlation_before = corr2(max(IM_Source_translate,[],3),IM_target_max)
 ...
 % Rigid
    FeaturePositions = load('C:\Users\Seyed\Documents\DatasetTests\GUI\NCT_Registration\MicroscopeFiles\Results-Neocortical2_StackList\MatchedPoints_Rigid.mat');
...
FeaturePositions = load('C:\Users\Seyed\Documents\DatasetTests\GUI\NCT_Registration\MicroscopeFiles\Results-Neocortical2_StackList\MatchedPoints_Affine.mat');
...
%     [X_aligned,L,b]=Global_Optimal_Affine(StackPositions_pixels,NewMatched); %Matched_Global
    ...
    FeaturePositions = load('C:\Users\Seyed\Documents\DatasetTests\GUI\NCT_Registration\MicroscopeFiles\Results-Neocortical2_StackList\StackPositions_Registered.mat');
...
%     translate_before = (NEW_StackPositions_pixels(sourceID,:) -NEW_StackPositions_pixels(targetID,:)) ;
%     IM_Source_translate = imtranslate(IM_Source,translate_before([2,1,3]));
%     figure;imshow(max(IM_Source_translate,[],3),[0 max(IM_source_max(:))]);
%     correlation_Translation = corr2(max(IM_Source_translate,[],3),IM_target_max)
...
r_Target_old = r_Target + [OLD_StackPositions_pixels(targetID,1),OLD_StackPositions_pixels(targetID,2),OLD_StackPositions_pixels(targetID,3)];
