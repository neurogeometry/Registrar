...
csvPath = 'TimeLapse_Holtmaat_StackList.csv';
...
%     figure(N);imshow(IMAll,[0 20])
    ...
    if sourceID == size(StackList,1)
        B1 = Boutons{sourceID-1,1};
        SourcePoints = B1.r2;
    ...
    r_source(:,2) = r_source(:,2) + 200;
    r_source(:,1) = r_source(:,1) + TR;
    figure(N); hold on; PlotAM(AM_source,r_source,color(TraceNum+sourceID-1,:))
    SourcePoints(:,1) = SourcePoints(:,1)+TR;
    SourcePoints(:,2) = SourcePoints(:,2) + 200;
    s = 1;
    for j=1:size(k)
        if f(j)< 3
    ...
    r_target(:,2) = r_target(:,2) + 200;
    r_target(:,1) = r_target(:,1) + TR;
    figure(N); hold on; PlotAM(AM_target,r_target,color(TraceNum+sourceID-1,:))
    TargetPoints(:,1) = TargetPoints(:,1)+TR;
    TargetPoints(:,2) = TargetPoints(:,2) + 200;
    s = 1;
    for j=1:size(k)
        if f(j)< 3
%             figure(N);hold on; plot(TargetPoints(j,2),TargetPoints(j,1),'or')
    ...
    for i=1:size(SourcePoints_onTrace,1)
        figure(N);hold on;line([SourcePoints_onTrace(i,2) TargetPoints_onTrace(i,2)], ...
            [SourcePoints_onTrace(i,1) TargetPoints_onTrace(i,1)], 'Color', rand(1,3));
